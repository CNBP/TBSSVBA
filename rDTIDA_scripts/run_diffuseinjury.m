...
if P1
    ...
    for i=1:size(brains,1)
        ...
        if tensor_bool
            deriveTensorandScalarMaps_dvlpt(fiddir,params_p1.tensor_est_method,ud,fid2,mask_path);
        end
    end
    dur_P1=toc(start_P1)
...
if P2
    ...
    params_p2.cur_orient=[4 6 5];
    params_p2.scmap_type={'RAD','B0','FA','MD','L1'};
    %----------------------------------------------------------------------
    %----------------------------------------------------------------------
...
if P3
    ...
    temp_cons_params.regex='SAL*.nii';
    ...
    %-----------------------------------------------------------------
    %-------------------------Parameters------------------------------
    atlas=['.' filesep 'Atlas' filesep 'atlas_p14.gipl'];
    
    reg_to_atlas.metric='MI'; %MODIF CECI N'A AUCUN EFFET
    ...
    if reg_to_atlas_bool
        fprintf(['\tRegistering template to atlas ' template ' ...\n'])
        try
