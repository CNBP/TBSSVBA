...
export ANTSPATH=${ANTSPATH:="$HOME/antsbin/bin/"} # EDIT THIS
...
#SGE=${ANTSPATH}/waitForSGEQJobs.pl LUIS
...
MAXITERATIONS=100x25x15x10 #customable_SyN
  ...
  then
    ...
    for (( i = 0; i < ${#IMAGESETARRAY[@]}; i+=$NUMBEROFMODALITIES ))
      do
        ...
        stage1="-t Rigid[0.1] ${IMAGEMETRICSET} -c [500x100x25x10,1e-8,10] -f 8x4x2x1 -s 4x2x1x0 -o ${outdir}/rigid${i}_" #customable_init_rig
        exe="${basecall} ${stage1}"
  ...
  do
    ...
    for (( j = 0; j < ${#IMAGESETARRAY[@]}; j+=$NUMBEROFMODALITIES ))
      do
        ...
        stage1="-t Rigid[0.1] ${IMAGEMETRICLINEARSET} -c [500x100x25x10,1e-8,10] -f 8x4x2x1 -s 4x2x1x0" #customable_rig
        stage2="-t Affine[0.1] ${IMAGEMETRICLINEARSET} -c [500x100x25x10,1e-8,10] -f 8x4x2x1 -s 4x2x1x0" #customable_aff
