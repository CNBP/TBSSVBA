function run_vbm_dti_analysis_wm_nonFA_v2_no_smoothing( IM_FOLDER,wm_mask,g1,g2,perm,VBM_FOLDER )
...
copyfile(IM_FOLDER,[IM_FOLDER '_wm'])
for i=1:length(imgs)
    
    infile=[IM_FOLDER '_wm' filesep imgs(i).name];
...
data_4d_file=[VBM_FOLDER filesep 'all_wm_' parts{end}];
%system(['source ~/.bash_profile ; fslmerge -t ' data_4d_file  ' `imglob ' IM_FOLDER '_wm_smooth' filesep '*.nii.gz`']);
system(['fsl4.1-fslmerge -t ' data_4d_file  ' `fsl4.1-imglob ' IM_FOLDER '_wm' filesep '*.nii.gz`']);
