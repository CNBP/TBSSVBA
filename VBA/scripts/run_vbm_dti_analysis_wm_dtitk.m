function run_vbm_dti_analysis_wm_dtitk(TENSOR_FOLDER, FA_FOLDER,template_file,fa_thresh,fwhm,g1,g2,perm,VBM_FOLDER )
...
tensors = dir(fullfile(TENSOR_FOLDER,'*.nii.gz'));

fileID=fopen([VBM_FOLDER filesep 'subjs.txt'],'w');

for i=1:length(tensors)
    
    fprintf(fileID,[TENSOR_FOLDER filesep tensors(i).name '\n']);
...
system(['source ~/.bash_profile; TVMean -in ' VBM_FOLDER filesep 'subjs.txt -out ' VBM_FOLDER filesep 'mean_tensor.nii.gz']);
system(['source ~/.bash_profile; TVtool -in ' VBM_FOLDER filesep 'mean_tensor.nii.gz -fa']);

%mean_nii=load_nii_gz([VBM_FOLDER filesep 'mean_tensor_fa.nii.gz']);
fprintf('Smoothing Mean FA image\n')
mean_FA_smoothed=gaussian_smooth_dtitk_tools([VBM_FOLDER filesep 'mean_tensor_fa.nii.gz'],fwhm);
% Note that smoothing the Mean FA or taking the Mean of smoothed images
% yields the same results since both processes are linear.
%% Derive mask (Mask on FA thresh and  on all images (tbss style))
min_img=min(img_all,[],4);
mask_whole=min_img>0;

mask_fa=double(mean_FA_smoothed>fa_thresh);
mask_nii=template;
